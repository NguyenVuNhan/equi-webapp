...
const ApplianceSchedule = ({ startTime }) => {
  const text = TimePrettier(startTime);
  const textLength = text.length * 17 + 30;

  return (
    <g
      style={{
        transform: `translateX(${439 - textLength}px) translateY(220px)`,
      }}
    >
      <mask
        id="ApplianceScheduleMask"
        mask-type="alpha"
        x="0"
        y="0"
      ...
      <g mask="url(#ApplianceScheduleMask)">
        <rect
          width={`${100 + textLength}`}
          height="64"
          rx="32"
          fill="url(#ApplianceScheduleColor)"
        />
        ...
        <text x="75" y="45" fontSize="35" fill="white">
          {text}
        </text>

        <defs>
          <linearGradient
            id="ApplianceScheduleColor"
            x1="0"
            y1="0"
            x2="154"
            y2="64"
