...
//=================================================
//  Rotator Encoder:  One Button, Multiple Events

int currentStateCLK = digitalRead(CLK);
int previousStateCLK = digitalRead(CLK);

void rotatorEncoder() {
  currentStateCLK = digitalRead(CLK);
  // If the previous and the current state of the
  // CLK are different then a pulse has occured
  if (currentStateCLK != previousStateCLK) {
    // If the DT state is different than the CLK
    // state then
    // the encoder is rotating counterclockwise
    if (digitalRead(DT) != currentStateCLK) {
      Serial.println("rotateRightEvent");
    } else {
      // Encoder is rotating clockwise
      Serial.println("rotateLeftEvent");
    }
  }
...
int debounce = 20;  // ms debounce period to prevent flickering when pressing or
                    // releasing the button
int DCgap = 250;    // max ms between clicks for a double click event
int holdTime = 500; // ms hold period: how long to wait for press+hold event
int longHoldTime = 3000; // ms long hold period: how long to wait for press+hold
                         // event
...
boolean longHoldEventPast = false; // whether or not the long hold event
                                   // happened already
...
int checkButton() {
  ...
  if (buttonVal == HIGH && (millis() - upTime) >= DCgap && DCwaiting == true &&
      DConUp == false && singleOK == true && event != 2) {
    event = 1;
    DCwaiting = false;
  }
